...
/// <summary>
/// The installed sink is process-wide, so the classes that swap it cannot run beside each other:
/// one would be asserting on lines the other's sink swallowed.
/// </summary>
...
/// <summary>
/// The record of what the host did while nobody was reading the screen. A meeting is not
/// reproducible — whatever went wrong happened once, in a room, with a supplier waiting — so the
/// log is the only place the answer can still be found afterwards.
/// </summary>
...
public class LoggingTests
{
    ...
    /// <summary>Restores whatever sink was installed, so one test cannot deafen the next.</summary>
    ...
    /// <summary>
    /// A full disk, a locked file, a log folder that was deleted mid-meeting: none of those may
    /// take the room down with them. Writing a line is never the operation that fails.
    /// </summary>
    [Fact]
    ...
    /// <summary>The four levels the operator can choose between, ordered quietest-last.</summary>
    [Fact]

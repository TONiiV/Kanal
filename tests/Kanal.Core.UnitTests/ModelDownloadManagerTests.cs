...
public class ModelDownloadManagerTests : IDisposable
{
    ...
    /// <summary>The transcription catalog (#72) reuses this downloader against records that are
    /// not <see cref="LocalModelInfo"/>.</summary>
    [Fact]
    public async Task DownloadsAnythingThatDeclaresAFileUrlSizeAndHash()
    {
        var handler = new FakeHandler();
        var manager = Manager(handler);
        IDownloadableFile part = new TestPart(
            FileName: "encoder.int8.onnx",
            DownloadUrl: "https://example.invalid/encoder.int8.onnx",
            SizeBytes: Payload.Length,
            Sha256: Convert.ToHexStringLower(SHA256.HashData(Payload)));

        Assert.False(manager.IsDownloaded(part));
        await manager.DownloadAsync(part, null, CancellationToken.None);

        Assert.True(manager.IsDownloaded(part));
        Assert.Equal(part.DownloadUrl, handler.LastUri!.ToString());
        Assert.Equal(Payload, await File.ReadAllBytesAsync(manager.GetPath(part)));
    }

    private sealed record TestPart(
        string FileName,
        string DownloadUrl,
        long SizeBytes,

...
public class LanguageLimitTests
{
    ...
    /// <summary>The catalog dialog must show the refusal, not swallow the click.</summary>
    [AvaloniaFact]
    public void CatalogDialogDisablesTheRowsThatCannotBeAdded()
    {
        var vm = AtTheCap();
        var dialog = new LanguagesWindow { DataContext = vm };
        dialog.Show();
        Dispatcher.UIThread.RunJobs();

        var boxes = dialog.GetVisualDescendants().OfType<CheckBox>()
            .Where(c => c.DataContext is LanguageOption)
            .ToDictionary(c => ((LanguageOption)c.DataContext!).Code, c => c);

        Assert.True(boxes["zh"].IsEnabled);
        Assert.False(boxes["fr"].IsEnabled);

        var notice = dialog.GetVisualDescendants().OfType<TextBlock>()
            .FirstOrDefault(t => t.Text is not null &&
                                 t.Text.Contains("four columns maximum", StringComparison.OrdinalIgnoreCase));

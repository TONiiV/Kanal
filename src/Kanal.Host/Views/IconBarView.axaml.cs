...
public partial class IconBarView : UserControl
{
    public IconBarView()
    {
        ...
        {
            if (DataContext is MainViewModel vm)
                vm.ConfirmConsent = (saveAudio, continuing) => AskAsync(vm, saveAudio, continuing);
        };
    }

    private async Task<bool?> AskAsync(MainViewModel vm, bool saveAudio, string? continuing)
    {

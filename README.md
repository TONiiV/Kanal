...
| Captions and room state | Sent through the authenticated Kanal gateway to a private Supabase Realtime channel. Joined phones receive transcript text, translations, speaker labels, language configuration, and pause/recording/lifecycle state. |
...
Kanal ships no Supabase project URL or client API key. The Edge Function is the only component that
holds a Supabase server credential or touches Realtime. The join QR contains the public gateway
address plus a receive-only, room-scoped ticket; anyone who obtains that bearer invitation can read
the room until the ticket expires (currently 12 hours), but cannot create rooms, publish messages,
enumerate other rooms, or use the project's database or storage through the gateway. Network
reachability for the relay is required for phones to receive captions.
...
| `KANAL_RELAY_URL` | Public HTTPS endpoint of the deployed `kanal-relay` Edge Function |
| `KANAL_RELAY_HOST_TOKEN` | At-least-32-byte host bootstrap secret; runtime only, never part of a build or QR |
...
Supabase client, project configuration, external font, or stylesheet dependency. On Start, the
...
- Mobile delivery depends on the authenticated gateway and Supabase Realtime being reachable. A
  different relay can be added behind `IRelayPublisher`, but no alternative ships today.

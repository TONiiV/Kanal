...
- **The open-source list, at the bottom of Settings.** An index of what Kanal is assembled from:
  project, licence name, and where to read it. Not the licence texts themselves — see the note below
  for what that leaves open. Each notice carries the NuGet ids it covers, and a test reads every shipped project file — `src/`, `tools/` and the root
...
### Decisions in that work a maintainer needs to keep

- **The log target is reconfigured in place, never replaced.** Handing NLog a second `FileTarget`
  over the file the first still holds open loses a contiguous block of what was already flushed —
  the new one opens at a stale offset. `ApplyTo` updates the live target's size, its archive count
  and the rule levels, and both `Program.Main`'s two applies and every Save go through that path.
- **Age is the retention policy; the archive count is only a runaway backstop.** NLog's file-count
  cap counts every file matching the pattern across dates, so a small fixed number silently cuts the
  14 days the panel promises down to hours; dropping it entirely let one loud day at a 1 MB rollover
  write 63 files and 65 MB with nothing yet eligible for deletion. The count is derived from the
...
- **A Save that cannot be written keeps the dialog open and says so.** It used to close as if it had
  ...
  file it came from, so it is removed on the next successful Save rather than kept forever.
- **The open-source list is scoped to the host binary.** Not `gateway/package.json` (devDependencies
  of a worker nobody is handed) and not the Supabase JS SDK the mobile page imports at runtime. A
  test holds it against every shipped project file read as XML, in both directions, and evaluates the
  `Condition` on an `IncludeAssets`/`PrivateAssets` exclusion rather than reading its text alone —
  the inverse condition would exempt a package that does ship.
...
wording here and in the README describes an index until that is decided. The changelog's prose is
also English on every language setting: translating release notes into four languages is a standing
cost on every release, not a bug fix.
